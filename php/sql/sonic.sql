...
CREATE TABLE `empresa` (
  ...
  `api_key` varchar(160) DEFAULT NULL,
  `Google_key` varchar(160) DEFAULT NULL,
  `autobk` varchar(160) DEFAULT NULL,
  ...
  `Gnet_url` varchar(255) DEFAULT NULL,
  `boletof_token` varchar(255) DEFAULT NULL,
  `boletof_url` varchar(255) DEFAULT NULL,
  `boletof_email` varchar(255) DEFAULT NULL,
...
INSERT INTO `empresa` (`id`, `empresa`, `endereco`, `numero`, `bairro`, `cidade`, `estado`, `cep`, `tel1`, `tel2`, `site`, `skype`, `email`, `dias_bloc`, `foto`, `contratocliente`, `contratoassinatura`, `cnpj`, `fistel`, `chave`, `regkey`, `banco`, `bancos_codigo`, `bancos_layout`, `smslogin`, `smssenha`, `emailspc`, `senhaspc`, `codtributo`, `ie`, `im`, `serie`, `modelonota`, `tipo`, `naturezaop`, `opsimples`, `ic`, `codmunicipio`, `codservico`, `apikey`, `api_key`, `autobk`, `receberate`, `instrucoes1`, `instrucoes2`, `instrucoes3`, `instrucoes4`, `token_gnet`, `Client_Id`, `Client_Secret`, `Gnet_url`, `boletof_token`, `boletof_url`, `boletof_email`, `carteira`, `agencia`, `digito_ag`, `conta`, `digito_co`, `tipo_carteira`, `convenio`, `convenio_dv`, `contrato`, `especie`, `lancarfaturas`, `obs`, `versao`, `avisotemporario`, `avisopermanente`, `whatsapp1`, `whatsapp2`, `qtddiasrenovacaoboleto`) VALUES
(1, 'Sonic ISP', 'RUA TAL', '45', 'Ponte Alta Norte', 'BRASILIA', 'DF', '71880-144', '0000', '00000', '', '', 'lamp@lampinc.com.br', 0, 'logo-sonic.png', 'contrato.rtf', NULL, '00.000.300/0001-23', '1234567890', '', '4084a9f1-235b-4506-a8ea-a683e9cbdcc8', '10', 104, 'cnab240_SIGCB', 'sonicisp', '898989899', '', 'CUVNxjmyL', '', '1234567890', '1234567890', 'A', '21', '1', '1', '1', '2', '', '', 'bbae2b725c369836424b0195dda34c1379385548', 'ux7QxA6x3S0dqHoNU2OfctVRl1yvCzYDPdJQXrfMNH4ELgENRFF', 'S', 'Não receber após 10 dias de vencimento', 'Cobrar 0% de Multa por dia', 'Em caso de dúvidas entre em contato conosco.', 'Sonic ISP, Sistema de gestão', NULL, '', '', '', '', '', '', 'true', '02', '4331', '0', '472102', '0', 'CR', '47210', '2', '000012', 'DM', '0', '', '1.2', '<h1 style="text-align: center;"><span style="color: #ff0000;">ATEN&Ccedil;&Atilde;O!</span></h1>\r\n<p style="text-align: center;"><strong>SUA INTERNET&nbsp;N&Atilde;O&nbsp;ESTA BLOQUEADA</strong></p>\r\n<p style="text-align: center;">&nbsp;</p>\r\n<p style="text-align: center;">Informativo:</p>\r\n<p style="text-align: center;">Prezado cliente,</p>\r\n<p style="text-align: center;">Constam boleto(s) em aberto em nosso sistema, favor regularize e evite o bloqueio de sua internet.</p>\r\n<p style="text-align: center;">Para acesso ao(s) boleto(s) clique aqui: Central do Assinante</p>\r\n<p style="text-align: center;">Ap&oacute;s 3 minutos feche o navegador e abra novamente para ter acesso a internet.</p>\r\n<p style="text-align: center;">&nbsp;</p>\r\n<p style="text-align: center;">&nbsp;Central de Atendimento &nbsp;(XX) XXXX-XXXX</p>', '<h1 style="font-family: -apple-system-font;" align="center"><span style="color: #ff0000;">CONEX&Atilde;O INTERROMPIDA!</span></h1>\r\n<h3 style="font-family: -apple-system-font; text-align: center;">Informativo:&nbsp;</h3>\r\n<h3 style="font-family: -apple-system-font; text-align: center;">Prezado cliente,</h3>\r\n<p style="font-family: -apple-system-font; font-size: 12px; line-height: 16px; text-align: center;">Consta(m) boleto(s) em aberto com data superior a 05 (CINCO) dias em nosso sistema. Acesso bloqueado conforme clausula contratual.</p>\r\n<p style="font-family: -apple-system-font; font-size: 12px; line-height: 16px; text-align: center;">Para acesso ao(s) boleto(s) clique aqui:&nbsp;<a href="http://localhost/central">Central do Assinante</a></p>\r\n<p style="font-family: -apple-system-font; font-size: 12px; line-height: 16px; text-align: center;">&nbsp;</p>\r\n<p style="font-family: -apple-system-font; font-size: 12px; line-height: 16px; text-align: center;">Central do Assinante:</p>', '', '', '12');
COMMIT;
...
(1, 'admin', '199283', NULL, 'admin', '202cb962ac59075b964b07152d234b70', 'MTIz', 'lamp@lampinc.com.br', '1', 'S'),
(2, 'Sonic ISP', NULL, NULL, 'sonic', '7f77bcdf5596fdadb002183b4b2942dc', 'MzMjQG15cm91dGVyQCMzMw==', 'admin@sonicisp.com.br', '1', 'S');
COMMIT;
...
BEGIN
    ...
    select distinct sonic.financeiro.situacao INTO sit
    from   sonic.financeiro
    left join  sonic.radcheck on (sonic.radcheck.pedido = sonic.financeiro.pedido)
    where radcheck.username           = login
    and   sonic.financeiro.situacao = 'B';
    ...
    if (sit = 'B') then
        ...
        from financeiro
        join  sonic.radcheck on (sonic.radcheck.pedido = sonic.financeiro.pedido)
        where sonic.radcheck.username = login
          and sonic.financeiro.status   = 'A'
          and sonic.financeiro.situacao = 'N'
          and sonic.financeiro.vencimento  <= date_add(curdate(), interval - 2 day) ;
    end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `postauth`(username VARCHAR(64), macaddress VARCHAR(17))
  BEGIN
    ...
    SELECT sonic.assinaturas.pedido INTO num_pedido FROM sonic.assinaturas WHERE login = username;
    ...
    IF NOT existe THEN
      ...
      UPDATE sonic.assinaturas SET mac = macaddress WHERE sonic.assinaturas.pedido = num_pedido;
    END IF;
  END
;;
delimiter ;
