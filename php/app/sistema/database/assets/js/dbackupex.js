...
// Show list
//###########################################################################*/
$('#backup_list_container').load('dbackupex_ajax.php?schow_all_backups=qwdwef6834r45z65g4wbackupsshow');

$(document).ready(function () {

 
/*========= Funcao notificacao  ========= */ 
function notifi_sucesso(text) {
new Noty({
  type: 'success',
  layout: 'bottomRight',
  theme: 'mint',
...
$(document).on('click', '.del-db-backup', function(e) {

var id = 'row_'+$(this).attr('row');
var file = $(this).attr('id');
$.ajax({
           type: "POST",
           url: "dbackupex_ajax.php",
           data: {"del_backup":'klar',"file":file},
		   beforeSend: function() { 
		   $('#backup_list_container').prepend('<div class="loader"></div>');
								  }, 
           ...
           success: function(data)
           {
              ...
              // Se a resposta conter (Error:) call a noti vermelho e nao verde
			 if(data.indexOf('<i class="fa fa-exclamation-triangle"></i>') > -1){notifi_erro(data);} else { notifi_sucesso(data);$('#'+id).fadeOut(800);}  
 
			  
			 $('.loader').fadeOut(500);/* Retirar loader */
...
$(document).on('click', '#create_new_backup', function(e) {
$.ajax({ 
	      type: "POST",
           url: "dbackupex_ajax.php",
           data: {"create_new_backup":'45zzg568htzu'},
		   beforeSend: function() { 
           ...
           success: function(data)
           {
              ...
              // Se a resposta conter (Error:) call a noti vermelho e nao verde
			 if(data.indexOf('<i class="fa fa-exclamation-triangle"></i>') > -1){notifi_erro(data);} else {notifi_sucesso(data);$('#backup_list_container').load('dbackupex_ajax.php?schow_all_backups=qwdwef6834r45z65g4wbackupsshow');}  
...
var file = $(this).attr('restoreid');
$.ajax({ 
	      type: "POST",
           url: "dbackupex_ajax.php",
           data: {"restore_backup":'23erf34f374h9f684d', "file":file},
		   beforeSend: function() { 
		   $('body').prepend('<div class="loader"></div>');
								  }, 
           ...
           success: function(data)
           { 
			 if(data.indexOf('<i class="fa fa-exclamation-triangle"></i>') > -1){notifi_erro(data);} else {notifi_sucesso(data);}  
...
$('.upload_new_backup').ajaxForm({
beforeSend: function() { 
		   $('form.upload_new_backup').prepend('<div class="loader_card"></div>');
								  }, 
 ...
 $(".upload_new_backup input[type=file]").on('change',function(){
   $( "form.upload_new_backup" ).submit(); 
});
 ...
 $( ".resete-tocken-button" ).click(function() {	
$.ajax({
           type: "POST",
           url: "dbackupex_ajax.php",
           data: {"resete_token":'3d4cdadqd564b45'},
		   beforeSend: function() { 
		   $('#cron_link_container').prepend('<div class="loader"></div>');
								  }, 
           ...
           success: function(data)
           {
              ...
              // Se a resposta conter (Error:) call a noti vermelho e nao verde
			 if(data.indexOf('<i class="fa fa-exclamation-triangle"></i>') > -1){notifi_erro(data);} 
			   else 
			   { 
				 $('#cron_link_container code').text(data);
				  $('#cron_link_container .copy-clip').attr("data-clipboard-text",data); 
			   }  
...
/*========= Show link to copy ========= */	
 $( ".copy-clip" ).click(function() {
  ...
  $(this).prev( "span.copied" ).fadeIn(100).delay(10).fadeOut(2000);	
    
});
/*========= END Show link to copy ========= */	
...
$('#set_backup_option').submit(function() {
  return false;
});	
	
	
$(document).on('click', '#set_backup_option a', function(e) {
$.ajax({ 
	      type: "POST",
           url: "dbackupex_ajax.php",
           data: $('#set_backup_option').serialize(),
		   beforeSend: function() { 
		   $('body').prepend('<div class="loader"></div>');
								  }, 
 ...
 /*========= Para o os links se abram dentro do ajax e nao no parent ========= */
$(document).on('click', '#backup_list_container .paginacao a ', function(e) {
	 e.preventDefault();
	$('#backup_list_container').prepend('<div class="loader_card"></div>');
 
    var url = $(this).attr("href");
    $('#backup_list_container').css("overflow","hidden").fadeOut(200).load(url).fadeIn(200);
 
	$('.loader_card').fadeOut(600);
	setTimeout(function() {
  $('.loader_card').remove();
}, 1000);
});	
